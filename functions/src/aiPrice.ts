...
export interface AIPriceInputData {
  cropName: string;
  category: string;
  quantity: number;
  qualityGrade: string;
...
export function validateAIPriceInput(data: any): AIPriceInputData {
...
export async function calculateAIPriceRecommendation(rawInput: any) {
...
export async function calculateDemandForecast(rawInput: any) {
  const data = validateAIPriceInput(rawInput);
  const apiKey = process.env.GEMINI_API_KEY;
