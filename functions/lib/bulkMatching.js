...
exports.performBulkMatching = performBulkMatching;
function performBulkMatching(cropName, requiredKg, buyerLat, buyerLng, candidates) {
    ...
    const getQualityScore = (grade) => {
        const g = (grade || '').toLowerCase();
        if (g.includes('export') || g.includes('organic'))
            return 100;
        if (g.includes('grade a'))
            return 90;
        if (g.includes('grade b'))
            return 75;
        return 60;
    };
    const scored = (candidates || [])
        ...
        const score = Math.round(quantityScore * WEIGHTS.quantity +
...
//# sourceMappingURL=bulkMatching.js.map